...
- (void)setCurrentSelectedIndex:(NSInteger)currentSelectedIndex {
    ...
    if (_currentSelectedIndex != currentSelectedIndex && currentSelectedIndex == finallPage) {
        ...
        if (lastSelectedButton.selected == YES) {
...
- (void)setTitleArray:(NSArray *)titleArray {
    ...
    [titleArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        //按钮距离下方scrollview的距离15
        btn.frame = CGRectMake(idx * pageWidth, 8, pageWidth, self.frame.size.height);
        btn.imageEdgeInsets = UIEdgeInsetsMake(11, -3, 11, 3);
        [btn setTitle:obj forState:UIControlStateNormal];
        [btn addTarget:weakSelf action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
        [weakSelf addSubview:btn];
        [weakSelf.allBtn addObject:btn];
    }];
}
...
- (void)setImageArray:(NSArray *)imageArray {
    ...
    for (int i = 0; i < imageArray.count; i ++) {
        UIButton *btn = self.allBtn[i];
        [btn setImage:[UIImage imageNamed:imageArray[i]] forState:UIControlStateNormal];
        NSLog(@"image %@",[UIImage imageNamed:imageArray[i]]);
