               ...
               WithImages: (NSMutableArray *) images
{
    ...
    if (self) {
        ...
        //当前显示页面
        _currentPage = 1;
        _imageViewcontentModel = UIViewContentModeLeft | UIViewContentModeRight;
...
-(void) addImageviewsForMainScrollWithImages: (NSMutableArray *) images{
    //设置ContentSize
...
#pragma mark - 把传进来的网址数组进行处理，把它转换成NSData对UIImage进行赋值
        //UIImage *imageTemp = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:imageName]]];
       // 使用SDWebImage进行图片缓存处理
       [tempImageView sd_setImageWithURL:[NSURL URLWithString:imageName] placeholderImage:[UIImage imageNamed:@"标志"]];
